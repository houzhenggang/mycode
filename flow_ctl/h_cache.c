...
static void scalar_timer_func(struct rte_timer *tim, void *data)
{
    ...
    if ((rte_rdtsc() - he->timestamp) / hz > he->timeo){
...
//	uint64_t cur_time = rte_get_timer_cycles();
//	struct h_bucket *b = he->bucket;
    if (((rte_rdtsc() - he->timestamp) / pv.hz > he->timeo)/* && rte_atomic16_test_and_set(&he->ref)*/){
 //       rte_spinlock_lock(&b->lock);
#if 0
...
#ifndef USE_HASH_TIMER
        if (he->timeo > 0 && (rte_rdtsc() - he->timestamp) / pv.hz > he->timeo) {
            rte_spinlock_lock(&b->lock);
            if (he->deleted == 0) {
                if (likely(__kfifo_put(pv.hash_do_queue[rte_lcore_id()],
                (const unsigned char *) &he, 8) == 8)) {
                    he->deleted = 1;
                }
		...
		void (*action)(struct h_scalar *, void *, void *) )
{
    ...
    cds_list_for_each_entry_rcu(he, &b->chain, list) {
        ...
        if (he->timeo > 0 && (rte_rdtsc() - he->timestamp) / pv.hz > he->timeo) {
            rte_spinlock_lock(&b->lock);
            if (he->deleted == 0) {
                if (likely(__kfifo_put(pv.hash_do_queue[rte_lcore_id()], (const unsigned char *)&he, 8) == 8)) {
                    he->deleted = 1;
                }
            }
            rte_spinlock_unlock(&b->lock);
